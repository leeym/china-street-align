...
1. **Detect region** — reads the map camera `@lat,lon` from the URL (not device GPS). If the point is outside the overlay region (approximate PRC land and near-coastal waters only — Taiwan Strait on/east of the ROC MND median from 27°N,122°E to 23°N,118°E then due south along 118°E; Taiwan / Japan / ROK ADIZ cuts over the Taiwan Strait, East China Sea, and Yellow Sea; Taiwan island, Hong Kong, Macau, neighboring countries, and open seas are excluded), or the GCJ→WGS screen shift is below ~3 px (too small to see when zoomed out), the extension does nothing.
